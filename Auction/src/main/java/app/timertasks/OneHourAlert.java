...
public class OneHourAlert extends TimerTask {
    String auction_id;
    Channel channel;
    MongoDB db;
    RemoteUserManagement rmiUser;

    public OneHourAlert(String auctionId, Channel channel, MongoDB db, RemoteUserManagement rmiUser){
    ...
    public void run(){
        try {
            ...
            System.out.println("Start one hour alert for: " + auction_id);
            List<String> emails = rmiUser.getEmailList(user_ids);
            for(int i = 0; i < user_ids.size(); i++){
                JsonObject jsonObj = new JsonObject();
                jsonObj.addProperty("type", "alert one hour");
